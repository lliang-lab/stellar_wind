	...
	/* to produce the emission line profiles */
	...
	#define vwcs 150 // v_wind in unit of c_s
	#define vdcs 5  // v_d in unit of c_s
	...
	int main(void)
	{
		...
		int i,j,k,m,bin,flag,index_min; 
		double xi,vmin, v_ws,phi;
		double r_0, A, B, C, gamma,T,y1,y2,y3,beta;
		double a1,a2,b1,b2,c1,c2;
		double *fterm1, *fterm2, *pl;
		double *vtemp1,*vtemp2,*vtemp3;
		double inten[101][4],couet[101][4], couet2[101][4];
		double mass, mass_ml, mass_ml2, cos1,sin1,cos2,sin2;
		FILE *velfile,*emfile, tempfile;
		//FILE *tmpfile;
	    
	    velfile=fopen("../data/ulrich_vel_c004w150d5c1c2c1y.out","w");
	    emfile=fopen("../data/ulrich_em_c004w150d5c1c2c1y.out","w");
	...
	//To make the emission line intensity plot
		for(i=0;i<101;i++){
			for(j=0;j<4;j++) {inten[i][j]=0.;couet[i][j]=0.;couet2[i][j]=0.;}
		}
		mass = 0.; mass_ml=0.; mass_ml2=0.;
	    
	//the viewing angles between the disc plane and the line-of-sight
		theta_obs[0] = 0.;
		theta_obs[1] = 0.524;
		theta_obs[2] = 1.047;
		theta_obs[3] = 1.571;
...
/////////           Emission Properties       ////////////
//////////////////////////////////////////////////////////
	double v_los, cos_phi, sin_phi, ds, sigma, sigma_ml; // sigmaml: surface density in the central mixing layer

	for(i=1;i<Nh;i++){

	v_ws=sigv2[i]/(1.e-20+sigv[i]); // velocity in the shocked wind layer in unit of v_wind
	//if(i != 1000) continue;
	if(x0 + dx * i > 200) break; //emission is calculated to below 100 r_d

		//if(i%1000==0) 
	cos1 = omega[i]/pow(1.+omega[i]*omega[i],0.5); // cosine beta
	sin1 = 1./pow(1.+omega[i]*omega[i],0.5); // sine beta 
	ds = dx * pow(1+omega[i]*omega[i],0.5); // dx is just dZ
	sigma = sigv[i]*sigv[i]/(1.e-20+sigv2[i]); // Sigma in shocked wind layer
						//sigv: mass flux in shocked wind layer; sigv2: momentum flux in shocked wind layer
	sigma_ml = rhol[i] * (h1[i] + h2[i]);

	fprintf(stderr,"i=%d Z=%g R=%g v1=%g cos(beta) = %g \n",i, x0 + dx * i, rad[i], v_ws, cos1);

	for(j=0;j<101;j++){

		//phi = 2*pi*j/100.;  //azimuth angle
		cos_phi = 2. * j / 101. - 1. + 1./101.;
		phi = acos(cos_phi);
		sin_phi = pow(1.- cos_phi * cos_phi,0.5);
            	//cos1 = omega[i]/pow(1.+omega[i]*omega[i],0.5); // cosine beta  
            	//sin1 = 1./pow(1.+omega[i]*omega[i],0.5); // sine beta
            
           	for(k=0;k<4;k++){ // four different viewing angles
                	//Couette: bin = (-0.05*m*sigv2[i]/(1.e-20+sigv[i])*cos(theta_obs[k])*cos1*cos(phi)+0.05*m*sigv2[i]/(1.e-20+sigv[i])*sin(theta_obs[k])*sin1+1.)/(2./100.); // (-v_wind to + v_wind)
			//bin = (-sigv2[i]/(1.e-20+sigv[i])*cos(theta_obs[k])*cos1*cos(phi)+sigv2[i]/(1.e-20+sigv[i])*sin(theta_obs[k])*sin1+1.); // (-v_wind to + v_wind)
	
			//Non-Couette Case (epsilon scales to density)
			v_los = - v_ws * (cos(theta_obs[k])*cos1*cos_phi + sin(theta_obs[k])*sin1);//approaching is negative
			bin = (v_los+1.)/(2./101.); // approaching is negative  

			fprintf(stderr,"phi = %g, cos(phi) = %g, v_los = %g, bin=%d \n", phi / 3.141596 * 180., cos_phi, v_los, bin);
                	if(bin<0) bin = 0;
                	if(bin>100) bin = 100;

			// d Phi = const. 
			//inten[bin][k] += 2 * pi * rad[i] * 0.01 *(dx*pow(1+omega[i]*omega[i],0.5))*sigv[i]*sigv[i]/(1.e-20+sigv2[i]);//*pow(rad[i]-omega[i]*(x0+dx*i),2)/(1.+omega[i]*omega[i])/pow(pow(x0+dx*i,2)+rad[i]*rad[i],2);

			// d Phi = d cos_phi / sin_phi
                	inten[bin][k] += 2. * rad[i] * (1./101.)/sin_phi * ds * sigma;//*pow(rad[i]-omega[i]*(x0+dx*i),2)/(1.+omega[i]*omega[i])/pow(pow(x0+dx*i,2)+rad[i]*rad[i],2);

	               	// Couette Case (epsilon scales to density)
                    	for (m=0;m<50;m++){
                		//Couette: 
				v_los = - 1./50. * m * v_ws * (cos(theta_obs[k])*cos1*cos_phi + sin(theta_obs[k])*sin1);//approaching is negative
                		bin = (v_los+1.)/(2./101.);
				if(bin<0) bin = 0;
				if(bin>100) bin = 100;
				couet[bin][k] += 1./50. * 2. * rad[i] * (1./101.)/sin_phi * ds * sigma_ml;
				couet2[bin][k] += 1./50. * 2. * rad[i] * (1./101.)/sin_phi * ds * sigma_ml * rhol[i];
                	}
            	}
        }
        mass += pi * rad[i] * ds * sigma;
	mass_ml += pi * rad[i] * ds * sigma_ml;
	mass_ml2 += pi * rad[i] * ds * sigma_ml * rhol[i];
    }
    
    fprintf(stderr,"start normalization.\n");
    
    for(i=0;i<101;i++){
        for(j=0;j<4;j++) {inten[i][j] = inten[i][j]/mass; couet[i][j] = couet[i][j]/mass_ml; couet2[i][j] = couet2[i][j]/mass_ml2;}
    }
    
    for(i=0;i<101;i++){
        fprintf(emfile,"%g %g %g %g %g %g %g %g %g %g %g %g %g\n", -1+i*0.02, inten[i][0],inten[i][1],inten[i][2],inten[i][3],couet[i][0],couet[i][1],couet[i][2],couet[i][3],couet2[i][0],couet2[i][1],couet2[i][2],couet2[i][3]);
    }
...
double f2(double x, double y1, double y2, double y3, double y4)
{
	...
	ram[num] = pram;
	vram2[num] = vr[num]*vr[num]+vth[num]*vth[num]; //th stands for theta
